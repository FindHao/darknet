...
__global__ void upsample_kernel2(size_t N, float *x, int w, int h, int c, int batch, int stride, int forward, float scale, float *out)
{
    size_t i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i >= N)
        return;
...
extern "C" void upsample_gpu2(float *in, int w, int h, int c, int batch, int stride, int forward, float scale, float *out)
{
    size_t size = w * h * c * batch * stride * stride;
    upsample_kernel2<<<cuda_gridsize(size), BLOCK, 0, get_cuda_stream()>>>(size, in, w, h, c, batch, stride, forward, scale, out);
    CHECK_CUDA(cudaPeekAtLastError());
}
